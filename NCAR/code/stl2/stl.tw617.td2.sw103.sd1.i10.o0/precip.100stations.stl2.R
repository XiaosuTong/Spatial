############################################################################
##Precipitation, 100 stations, remainder+seasonal component, remainder, 
##trend loess, trend with yearly average AGAINST time.
##########################################################################
...
dr <- ddply(tmp, "station.id", function(r){stl2(r$precip, r$date, n.p=12, s.window=103, s.degree=1, t.window=617, t.degree=2, inner = 10, outer = 0)$data})
tmp <- cbind(tmp, dr)
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_with_stl2_trend+seasonal_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
   for(i in stations){
...
##################################################################################
#QQ plot and time series of Pooled remainder of precipitation for 100 stations
##################################################################################

#Create the QQ plot of precipitation conditional on month for one station
trellis.device(postscript, file = paste(outputdir, "QQ_plot_of_stl2_remainder_precipitation_of_100_stations", ".ps", sep = ""), color=TRUE, paper="legal")
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_with_stl2_remainder_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
   for(i in stations){
        b <- xyplot( remainder ~ time | factor,
             data = subset(tmp.remainder, station.id==i),
             xlab = list(label = "Month", cex = 1.2),
             ylab = list(label = paste("Station",i,"Precipitation (millimeters)"), cex = 1.2),
#             main = list(label = paste("Station ", i, sep=""), cex=1.5),
             type = "p",
             layout = c(1,3),
             pch = 16,
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_with_stl2_remainder2_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
   for(i in stations){
...
trellis.device(postscript, file = paste(outputdir, "acf_of_precipitation_with_stl2_remainder_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
   for(i in stations){
...
#
#trellis.device(postscript, file = paste("scatterplot_of_precipitation_with_stl2_seasonal+remainder_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
#   for(i in stations){
...
#             panel = function(...) {
#                  panel.abline(h=seq(-50,80,by=10), v=seq(0,140, by=12), color="lightgrey", lty=3, lwd=0.5)
#                  panel.xyplot(...)
#
#             }
#        )
...
#########################################################################
##Scatter plot of trend component and yearly average of raw from stl2
#########################################################################
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_with_stl2_trend_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
...
#            scales = list(y = list(relation = 'free', cex=1.5), x=list(relation= 'same',format = "%b %Y", tick.number=8), cex=1.2),
             ...
             panel = function(x,y,...) {
                  panel.abline(h=seq(-2,40, by=2), v=seq(0, 1236, by=120), color="lightgrey", lty=3, lwd=0.5)
...
#
#trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_with_stl2_trend+remainder_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
#   for(i in stations){
#        b <- xyplot( response ~ time,
...
#             panel = function(x,y,...) {
#                  panel.abline(h=seq(0,80,by=10), v=seq(0,1236, by=120), color="lightgrey", lty=3, lwd=0.5)
#                  panel.xyplot(x,y,...)
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_with_stl2_trend_loess_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
    for(i in levels(tmp.trend$station.id)){
        b <- xyplot( trend ~ time,
...
#            scales = list(y = list(relation = 'free', cex=1.5), x=list(relation= 'same',format = "%b %Y", tick.number=8), cex=1.2),
             ...
             panel = function(x,y,...) {
                  panel.abline(h=seq(-2,40,by=1), v=seq(0,1236, by=120), color="lightgrey", lty=3, lwd=0.5)
...
#us.map <- map('state', plot = FALSE, fill = TRUE)
#
#trellis.device(postscript, file = paste(outputdir, "QQ_plot_of_precip_slope_of_trend_loess",".ps", sep = ""), color=TRUE, paper="legal")
#    for(i in station.or){
#        a <- qqmath(~ slope,
...
#trellis.device(postscript, file = paste(outputdir, "scatter_plot_of_average_slopes_precip",".ps", sep = ""), color=TRUE, paper="legal")
