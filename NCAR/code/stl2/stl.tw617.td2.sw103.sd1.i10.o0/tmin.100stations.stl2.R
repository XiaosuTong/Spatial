...
dr <- ddply(tmp, "station.id", function(r){stl2(r$tmin, r$date, n.p=12, s.window=103, s.degree=1, t.window=617, t.degree=2, inner = 10, outer = 0)$data})
tmp <- cbind(tmp, dr)
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_tmin_with_stl2_remainder2_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
   for(i in stations){
...
trellis.device(postscript, file = paste(outputdir, "acf_of_tmin_with_stl2_remainder_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
   for(i in stations){
