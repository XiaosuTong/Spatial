#######################################################################################
##Min temperature, 100 stations, loess on each component from stl2 conditional on month
#######################################################################################
...
dr <- ddply(tmp, "station.id", function(r){stl2(r$tmin, r$date, n.p=12, s.window=103, d.degree=1, t.window=617, t.degree=2, inner = 10, outer = 0)$data})
tmp <- cbind(tmp, dr)
...
#order the 100 stations of Min temperature by the elevation.
...
#            	.fun = summarise,
#            	 year = c(1895:1997),
#            	 fitted = loess(seasonal~year, degree=1, span=1/3)$fitted)
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_tmin_seasonal_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
...
#             main = list(label= paste("Station ", i, sep=""), cex=1.5),
             ...
             ylab = list(label = paste("Station",i,"Minimum Temperature (degrees centigrade)"), cex = 1.2),
             type = c("p"),
             pch=16,
             cex=0.5,
             layout = c(12,1),
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_tmin_seasonal2_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
        b <- xyplot( seasonal ~ year | month,
             data = subset(tmp.seasonal,station.id==i),
#             main = list(label= paste("Station ", i, sep=""), cex=1.5),
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_tmin_remainder_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_tmin_remainder2_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
        b <- xyplot( remainder ~ year | month,
             data = subset(tmp.remainder,station.id==i),
             xlab = list(label = "Year", cex = 1.2),
...
trellis.device(postscript, file = paste(outputdir, "QQ_plot_of_tmin_remainder_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
    for(i in stations){
...
#trellis.device(postscript, file = paste(outputdir, "scatterplot_of_tmin_seasonal+remainder_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
#     for(i in stations){
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_tmin_seasonal+remainder_nomean_same_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
        b <- xyplot( response ~ year | month,
...
#trellis.device(postscript, file = paste(outputdir, "scatterplot_of_tmin_seasonal+remainder_nomean_free_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
#     for(i in stations){
#        b <- xyplot( response ~ year | month,
#             data = subset(tmp.sr.mean, station.id==i),
#             groups = group,
