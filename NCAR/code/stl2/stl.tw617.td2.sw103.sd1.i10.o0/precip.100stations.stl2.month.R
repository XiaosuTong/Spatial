...
#order the 100 stations of precipitation by the elevation.
...
#            	.fun = summarise,
#            	 year = c(1895:1997),
#            	 fitted = loess(seasonal~year, span = 1/3, degree=1)$fitted)
#
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_seasonal_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
...
#            scales = list(y = list(relation = 'free', cex=1.5), x=list(relation= 'free',format = "%b %Y", tick.number=10), cex=1.2),
             ...
             panel = function(x,y,...) {
                  ...
                  panel.xyplot(x,y,...)
		  panel.loess(x,y,degree=1, span=3/4, col=col[2],...)
             }
        )
	print(b)
#        a <- xyplot(lat ~ lon,
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_seasonal2_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
        b <- xyplot( seasonal ~ year | month,
             data = subset(tmp.seasonal,station.id==i),
             xlab = list(label = "Year", cex = 1.2),
...
#                .fun = summarise,
#                 year = c(1895:1997),
#                 fitted = loess(remainder~year, span=1/3, degree=1)$fitted)
#
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_remainder_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_remainder2_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
        b <- xyplot( remainder ~ year | month,
             data = subset(tmp.remainder,station.id==i),
             xlab = list(label = "Year", cex = 1.2),
...
trellis.device(postscript, file = paste(outputdir, "QQ_plot_of_precip_remainder_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
    for(i in stations){
...
#tmp is ordered by station.id, month, and year.
...
#with mean with free scale
#trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_seasonal+remainder_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
#     for(i in stations){
...
#             scales = list(y = list(relation = 'same', alternating=TRUE), x=list(tick.number=10, relation='same')),
#             panel = function(x,y,...) {
#                  panel.abline(h=seq(-60,60,by=10), v=seq(1900,2000,by=10), color="lightgrey", lty=3, lwd=0.5)
#                  panel.xyplot(x,y,...)
#             }
#        )
#        print(b)
...
trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_seasonal+remainder_nomean_same_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
     for(i in stations){
        b <- xyplot( response ~ year | month,
...
#trellis.device(postscript, file = paste(outputdir, "scatterplot_of_precipitation_seasonal+remainder_nomean_free_for_100_stations_conditional_month",".ps", sep = ""), color=TRUE, paper="legal")
#     for(i in stations){
#        b <- xyplot( response ~ year | month,
#             data = subset(tmp.sr.mean, station.id==i),
#             groups = group,
