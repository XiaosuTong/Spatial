...
#            scales = list(y = list(relation = 'free', cex=1.5), x=list(relation= 'free',format = "%b %Y", tick.number=10), cex=1.2),
             ...
             panel = function(x,y,...) {
                  panel.abline(h=seq(-60,60,by=5), v=seq(1900,2000,by=10), color="lightgrey", lty=3, lwd=0.5)
                  panel.xyplot(x,y,...)
		  panel.loess(x,y,degree=1, span=1/3, col=col[2],...)
             }
        )
	print(b)
#        a <- xyplot(lat ~ lon,
...
#             scales = list(y = list(relation = 'free', cex=1.5), x=list(relation= 'free',format = "%b %Y", tick.number=10), cex=1.2),
             ...
             panel = function(x,y,...) {
                  ...
                  panel.xyplot(x,y,...)
		  panel.loess(x, y, degree=2, span=1/3, col=col[2],...)
             }
        )
        print(b)
     }
