...
dr <- ddply(tmp, "station.id", function(r){stl2(r$tmax, r$date, n.p=12, s.window=15, inner = 5, outer = 1)$data})
tmp <- cbind(tmp, dr)

tmp <- tmp[order(tmp$station.id,tmp$month),]
tmp <- tmp[,c(1:10,13,14,15)]

estimate.max.QQ <- function(tmp, station, dist){
...
##The station for this condition is 120200 with lon=-84.98 lat=41.63
	...
	remainder.est <- temp$tmax - temp$est.seasonal - temp$est.trend

       	a <- qqmath(~ remainder.est,
                distribution = qnorm,
                main = list(label= paste("QQ plot of remainder of max temperature for station ", station, sep=""), cex=2),
...
estimate.max.scatter <- function(tmp, station, dist){
	...
	b <- xyplot( response ~ date | factor,
             ...
             main = list(label = paste("Max Temperature for station ", station, sep=""), cex=1.5),
             type = c("p","l"),
...
trellis.device(postscript, file = paste("scatterplot_of_tmax_with_stl2_estimate_for_100_stations",".ps", sep = ""), color=TRUE, paper="legal")
    for(i in unique(tmp$station.id)){
	estimate.max.scatter(tmp, i, 4)
    }
dev.off()

trellis.device(postscript, file = paste("QQ_plot_of_stl2_estimate_emainder_tmax_of_100_stations", ".ps", sep = ""), color=TRUE, paper="letter")
    for(i in unique(tmp$station.id)){
	estimate.max.QQ(tmp, i, 4)
    }
dev.off()
