...
trellis.device(postscript, file = paste(outputdir, "QQ_plot_of_elevation", ".ps", sep = ""), color=TRUE, paper="legal")
		...
		grib = FALSE,
                ...
                panel = function(x, y,...) {
			panel.abline( h= dr.spatial$elev, color="lightgrey", lty=3, lwd=0.5)
                        panel.qqmath(x, y,...)
                }
...
#######################################################################################################
##Or find the 100 stations which have the largest observation # for both temperature and precipitation
########################################################################################################
tmp <- UStemp[(UStemp[1:8125,]$station.id %in% USppt[1:11918,]$station.id),]

station <- as.character(unique(tmp$station.id))

count1 <- ddply(.data = subset(USppt, station.id %in% station),
	.variables = "station.id",
	.fun = summarise,
	 count.p = sum(!is.na(precip))
	)

count2 <- ddply(.data = subset(UStemp, station.id %in% station),
        .variables = "station.id",
        .fun = summarise,
...
b <- xyplot(lat ~ lon | group,
                ...
                panel = function(x,y,...) {
                        panel.polygon(us.map$x,us.map$y)
						for(i in 1:length(x)){
							panel.text(x[i],y[i],labels=data[which(data$lon==x[i]&data$lat==y[i]),]$station.id, col="blue", adj=c(0,0), cex=0.5)
						}
                        panel.xyplot(x,y,...)
                }
)
