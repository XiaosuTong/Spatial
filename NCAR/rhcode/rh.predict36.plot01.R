...
trellis.device(postscript, file = file.path(local.output, paste("QQ_plot_of_mean_abserror_", dataset, ".ps", sep="")), color=TRUE, paper="legal")
	...
	print(b)
dev.off()
trellis.device(postscript, file = file.path(local.output, paste("scatter_plot_of_mean_abserror_", dataset, "_vs_lag_sw.ps", sep="")), color=TRUE, paper="legal")
	b <- xyplot( absmeans ~ lap | sw*as.factor(station.id),
             data = combined,
             xlab = list(label = "Lag", cex = 1.2),
             ylab = list(label = ylab, cex = 1.2),
             groups = as.factor(tw),
             key=list(type="l", text=list(label=levels(as.factor(combined$tw))),  lines=list(lwd=1.5, col=col[1:3]), columns=3),
             type = "b",
...
trellis.device(postscript, file = file.path(local.output, paste("scatter_plot_of_mean_abserror_", dataset,"_vs_lag_tw.ps", sep="")), color=TRUE, paper="legal")
	b <- xyplot( absmeans ~ lap | tw*as.factor(station.id),
             ...
             ylab = list(label = ylab, cex = 1.2),
             groups = as.factor(sw),
             key=list(type="l", text=list(label=levels(as.factor(combined$sw))),  lines=list(lwd=1.5, col=col[1:3]), columns=3),
             type = "b",
             scales = list(x=list(at=seq(from=0, to=36, by=6)), y=list(relation="sliced")),
             layout = c(3,1),
             pch=1,
...
## conditional on group for each station.
#################################################################################
trellis.device(postscript, file = file.path(local.output, paste("scatter_plot_of_mean_error_", dataset, "_vs_lag_sw.ps", sep="")), color=TRUE, paper="legal")
...
## conditional on group for each station.
#################################################################################
trellis.device(postscript, file = file.path(local.output, paste("scatter_plot_of_std_error_", dataset, "_vs_lag_sw.ps", sep="")), color=TRUE, paper="legal")
...
trellis.device(postscript, file = file.path(local.output, paste("dotplot_of_overmean_error_", dataset,"_tw.ps", sep="")), color=TRUE, paper="legal")
	b <- dotplot( overmean ~ station.id | tw,
             ...
             groups = sw,
             key=list(type="p", text=list(label=levels(as.factor(result$sw))),  points=list(col=col[1:3]), columns=3, pch=1),
             type = "p",
             layout = c(1,1),
...
trellis.device(postscript, file = file.path(local.output, paste("dotplot_of_overmean_error_", dataset,"_sw.ps", sep="")), color=TRUE, paper="legal")
        b <- dotplot( overmean ~ station.id | sw,
             data = result,
             ylab = list(label = "Mean of absolute prediction error", cex = 1.2),
             xlab = list(label = "Stations", cex = 1.2),
             groups = as.factor(tw),
             key=list(type="l", text=list(label=levels(as.factor(result$tw))),  points=list(pch=1, col=col[1:3]), columns=3),
             type = "p",
             layout = c(1,1),
             scales = list(x=list(draw=FALSE), y=list(relation="free")),
#                  panel.xyplot(x,y,...)
...
trellis.device(postscript, file = file.path(local.output, paste("dotplot_of_overstd_error_", dataset,"_sw.ps", sep="")), color=TRUE, paper="legal")
