...
trellis.device(postscript, file = file.path(local.output, paste("QQ_plot_of_mean_abserror_", dataset, ".ps", sep="")), color=TRUE, paper="legal")
	...
	print(b)
dev.off()
trellis.device(postscript, file = file.path(local.output, paste("scatter_plot_of_mean_abserror_", dataset, "_vs_lag.ps", sep="")), color=TRUE, paper="legal")
	b <- xyplot( absmeans ~ lap | as.factor(station.id),
             data = combined,
             xlab = list(label = "Lag", cex = 1.2),
             ylab = list(label = ylab, cex = 1.2),
             groups = group,
             key=list(type="l", text=list(label=c("two frequency components","one frequency component")),  lines=list(lwd=1.5, col=col[1:length(unique(combined$group))]), columns=length(unique(combined$group))),
             type = "b",
...
## conditional on group for each station.
#################################################################################
trellis.device(postscript, file = file.path(local.output, paste("scatter_plot_of_mean_error_", dataset, "_vs_lag.ps", sep="")), color=TRUE, paper="legal")
...
## conditional on group for each station.
#################################################################################
trellis.device(postscript, file = file.path(local.output, paste("scatter_plot_of_std_error_", dataset, "_vs_lag.ps", sep="")), color=TRUE, paper="legal")
...
trellis.device(postscript, file = file.path(local.output, paste("dotplot_of_overmean_error_", dataset,".ps", sep="")), color=TRUE, paper="legal")
	b <- dotplot( overmean ~ station.id,
             data = result,
             ylab = list(label = "Mean of absolute prediction error", cex = 1.2),
             xlab = list(label = "Stations", cex = 1.2),
             groups = group,
             key=list(type="p", text=list(label=c("two frequency components","one frequency component")),  points=list(col=col[1:length(unique(result$group))]), columns=length(unique(result$group)), pch=1),
             type = "p",
             layout = c(1,1),
...
trellis.device(postscript, file = file.path(local.output, paste("dotplot_of_overstd_error", dataset,".ps", sep="")), color=TRUE, paper="legal")
...
job$parameters <- list(parameter = parameter, ylab = ylab, div.stations= get(paste(index, "div.stations", sep=".")))
