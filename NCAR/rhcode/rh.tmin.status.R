...
reduce <- expression(
   ...
   post={
	library(lattice)
      	trellis.device(postscript, file = paste(local.output, "tmin_measurement_status_against_month_part_",reduce.key,".ps", sep = ""), color=TRUE, paper="legal")
        b <- dotplot( as.numeric(obs) ~ time | station.id,
             data = combined,
             xlab = list(label = "Month", cex = 1.5),
             ylab = list(label = "Minimum Temperature Measurement Status", cex = 1.5),
             type = "p",
             col = c("black","red"),
...
#             scales = list(y = list(relation = 'same', cex=1.2, alternating=TRUE), x=list(relation= 'same',format = "%b %Y", tick.number=10), cex=1.2),
        ...
        print(b)
	dev.off()
	system(paste("chmod 777 ", local.output, "tmin_measurement_status_against_month_part_",reduce.key,".ps", sep = ""))
   }
)
...
ex <- rhwatch(
        map = map,
        reduce = reduce,
        input = rhfmt(paste(rh.datadir,"tmin",sep=""), type="sequence"),
        output = rhfmt(paste(rh.output,"tmin",sep=""), type="sequence"),
