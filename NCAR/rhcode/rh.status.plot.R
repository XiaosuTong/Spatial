...
job$reduce <- expression(
  ...
  post={
    trellis.device(
      device = postscript, 
      file = paste("./tmp/", par$dataset, "_measurement_status_against_month_part_", reduce.key, ".ps", sep = ""), 
      color = TRUE, 
      paper = "legal"
    )
      b <- dotplot( as.numeric(obs) ~ time | station.id
        ...
        , strip = FALSE,
        , scales = list(
            y = list(relation = 'same', labels = c("0","1"), alternating = TRUE), 
            x = list(tick.number = 10, relation = 'same', cex = 1.2)
          )
        , panel = function(x,y,...) {
            panel.abline(v = seq(0,1235, by=60), col = "lightgrey", lty = 3, lwd = 0.5)
            panel.dotplot(x,y,...)
          }
      )
      print(b)
