...
job$map <- expression({
  lapply(seq_along(map.values), function(r) {
	...
	tmp <- llply(1:lap, function(i){
   	  do.call(rbind, llply(.data = kd003$cells, 
		.fun = function(r){
           	if(nrow(r) >= i)
	...
	lapply(1:length(tmp), function(r){
        ...
        value <- predict(lo.fit, data.frame(lon = kd003$vertix$lon, lat = kd003$vertix$lat))
...
job$input <- c(N, 242) 
job$output <- rhfmt(file.path(rh.datadir, dataset, "spatial", "DR.loess"), type="sequence")
