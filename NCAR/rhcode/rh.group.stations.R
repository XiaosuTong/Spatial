...
job$output <- rhfmt(file.path(rh.datadir, dataset,"100stations","sharepredict",index,"orderstations"), type="sequence")
...
job$parameters <- list()
job$input <- rhfmt(file.path(rh.datadir, dataset,"100stations","sharepredict",index,"orderstations"), type="sequence")
job$output <- rhfmt(file.path(rh.datadir, dataset,"100stations","sharepredict",index,"group.orderstations"), type="sequence")
job$mapred <- list(mapred.reduce.tasks = 72)
job$jobname <- paste(dataset, "order stations")
job$readback <- FALSE
job$mon.sec <- 10
...
rst <- rhread(file.path(rh.datadir, dataset, "100stations","sharepredict",index,"group.orderstations"))
assign(paste(index, "div.stations", sep="."), rst)
}
save(list=grep("div.stations", ls(), value=TRUE), file=paste(datadir, dataset, "div.stations.RData", sep=""))
