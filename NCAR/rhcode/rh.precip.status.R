...
#load(paste(local.datadir,"USmonthlyMet.RData", sep=""))

map <- expression({
      	lapply(seq_along(map.values), function(r) {
      	v <- map.values[[r]]
      	k <- map.values[[r]]$partion[1]
      	v$obs <- !is.na(v$precip)
	month <- v$month
...
reduce <- expression(
   ...
   post={
	library(lattice)
      	trellis.device(postscript, file = paste(local.output, "precipitation_measurement_status_against_month_part_",reduce.key,".ps", sep = ""), color=TRUE, paper="legal")
        b <- dotplot( as.numeric(obs) ~ time | station.id,
             data = combined,
             xlab = list(label = "Month", cex = 1.5),
             ylab = list(label = "Precipitation Measurement Status", cex = 1.5),
             type = "p",
             col = c("black","red"),
...
#             scales = list(y = list(relation = 'same', cex=1, labels=c("0","1"), alternating=TRUE), x=list(tick.number = 10, relation='same', cex=1.2)),
        ...
        print(b)
	dev.off()
	system(paste("chmod 777 ", local.output, "precipitation_measurement_status_against_month_part_",reduce.key,".ps", sep = ""))
   }
)
...
ex <- rhwatch(
        map = map,
        reduce = reduce,
        input = rhfmt(paste(rh.datadir,"precip",sep=""), type="sequence"),
        output = rhfmt(paste(rh.output,"precip",sep=""), type="sequence"),
