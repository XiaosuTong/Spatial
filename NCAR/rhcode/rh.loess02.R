...
job$map <- expression({
  lapply(seq_along(map.values), function(r) {
	...
	m <- month[j]
	y <- i + 1894
	v <- subset(get(dataset), year == y & month == m)[, c("station.id", "elev", "lon", "lat", dataset)]
	v <- na.omit(v)
	kd <- kdtree(v[,c("station.id","lon","lat")], alpha=0.05)
...
job$parameters <- list(dataset = dataset, kdtree = kdtree)
job$input <- c(N, 242) 
job$output <- rhfmt(file.path(rh.datadir, dataset, "spatial", "loess02"), type="sequence")
